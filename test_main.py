...
def test_read_jobs(test_client):
    ...
    for job in jobs:
        assert set(job.keys()) == set(test_job.keys()) - {
            "_id"
        }, "Job structure does not match expected format"

    # Assert presence of the test job in the response
    assert any(
        job for job in jobs if job["link"] == "http://test.com/test_job"
    ), "Test job not found in response"


def test_increment_outreach(test_client):
    # Define the request data
    update_data = {
        "job_link": test_job["link"],
        "outreach_type": "peer",
        "action": "increment",
    }
...
def test_outreach_nonexistent_job(test_client):
    update_data = {
        "job_link": "http://test.com/nonexistent_job",
        "outreach_type": "peer",
        "action": "increment",
    }
    response = test_client.put("/outreach/update_outreach", json=update_data)
...
def test_update_status_success(test_client):
    ...
    response = test_client.put(
        f"/jobs/update/status?job_link={job_link}&action={action}"
    )
    assert response.status_code == 200, "Expected status code to be 200 OK"
    assert response.json() == {
        "msg": f"Job marked {action}"
    }, "Expected success message not returned"

...
class OutreachUpdate(BaseModel):
    ...
    outreach_type: str = Field(
        ..., description="The type of outreach ('peer', 'manager', 'recruiter')"
    )
    action: str = Field(
...
class JobListing(BaseModel):
    ...
    applied: Union[bool, datetime] = Field(
        False, description="Indicates if the job has been applied to, and when"
    )
    status: Optional[str] = Field(
        None,
        description="Current status of the job application ('applied', 'deleted', 'rejected')",
    )
    outreach: Optional[OutReachCounts] = Field(
        default_factory=OutReachCounts, description="Counts of outreach interactions"
    )
...
async def create_job(job_data: JobListing, jobs_collection=Depends(get_db)):
    ...
    if existing_job:
        raise HTTPException(
            status_code=400, detail="Job with the given link already exists"
        )
